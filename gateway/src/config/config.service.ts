...
export class ApiConfigService {
    ...
    getGatewayPort(): number {
        return parseInt(this.configService.get('API_GATEWAY_PORT'), 10) ?? 3000
    }

    getUserServiceConfig(): TcpClientOptions {
        return {
            options: {
                port: parseInt(this.configService.get('USER_SERVICE_PORT'), 10) ?? 3001,
    ...
    getMeetupServiceConfig(): TcpClientOptions {
        return {
            options: {
                port: parseInt(this.configService.get('MEETUP_SERVICE_PORT'), 10) ?? 3002,
